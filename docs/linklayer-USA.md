...
(Same as euro version) The control byte 1 gives basic information about the frame (bit positions in the header):
...
Length of the frame in bytes, not including the header and this control byte or the two-byte CRC
...
After the payload, there is a 16-bit CRC.  In most cases this is calculated automatically by the transceiver IC.
...
In both modes (1-way/2-ways), the transferred key is obfuscated using different methods. But both methods imply a shared key probably specified in the protocol documentation. This key is referred to as **transfer key** and has the following value, probably hardcoded in the specification:
...
* Generate an initial vector (IV)
* Encrypt it using AES-128 and either the transfer key or the system key depending on the data encrypted
